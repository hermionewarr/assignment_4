...
void galaxy::print_data()
{
	...
	std::cout << "galaxy: [type ,h,sm,r,tm] = [" << hubble_type << "," << stellar_mass
		<< "," << redshift << "," << total_mass << "]" << std::endl;
	return;
...
int main()
{
	...
	g1.print_data();
	trial.print_data();
	for (size_t i{}; i < galaxy_data.size(), i++) {
		std::cout << "galaxy data: \n" << galaxy_data << std::endl;
	}
